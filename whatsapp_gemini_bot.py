...
print(f"[startup] Initializing Gemini client...")
try:
    client = genai.Client(api_key=GEMINI_API_KEY)
    print(f"[startup] Gemini client created OK")
except Exception as e:
    print(f"[startup] FAILED to create Gemini client: {e}")
    traceback.print_exc()
...
def get_gemini_reply(sender_id, user_message):
    ...
    try:
        print(f"[gemini] Sending to Gemini: {user_message[:50]}")
        ...
        print(f"[gemini] Got reply: {reply[:80]}")
    except Exception as e:
...
def bot_loop():
    ...
    while True:
        try:
            ...
            print(f"[bot_loop] Unexpected error: {e}")
            traceback.print_exc()
